...
Prosedur adalah sekumpulan instruksi yang dikelompokkan untuk menyederhanakan kompleksitas kode dalam program besar. Berbeda dengan fungsi, prosedur tidak memiliki deklarasi tipe nilai kembalian dan tidak menggunakan kata kunci `return`. Saat dipanggil dalam program utama, prosedur langsung menjalankan tugasnya tanpa mengembalikan nilai. Kedudukannya setara dengan instruksi dasar atau fungsi dari pustaka tertentu, seperti `fmt.Scan` dan `fmt.Print` dalam Go. Oleh karena itu, nama prosedur sebaiknya menggunakan kata kerja atau merepresentasikan suatu proses agar lebih jelas dan sesuai dengan fungsinya. Penggunaan prosedur membantu menciptakan kode yang lebih terstruktur, modular, dan mudah dipahami.
